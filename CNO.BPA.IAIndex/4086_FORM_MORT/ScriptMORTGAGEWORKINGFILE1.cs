...
namespace Custom.InputAccel.UimScript
{
    ...
    public class ScriptMORTGAGEWORKINGFILE1 : UimScriptDocument
    {
        private const string _validationRule = "One";

        public ScriptMORTGAGEWORKINGFILE1()
            : base()
        {
        ...
        private void AutoValidation(IUimDataContext dataContext)
        {
            try
            {
                ...
                //validate if only one field among UCC_FILE, BORROWER_NAME_INFO, LEGAL_DOCUMENTS NOT_CATEGORIZED is selected
                ...
                else
                {
                    ...
                    dataContext.FindFieldDataContext("DESCRIPTION2").SetValue("WORKING FILE I SEC");
                ...
                //UCC File
                if (dataContext.FindFieldDataContext("UCC FILE").Text.Trim().Length == 0)
                {
                    intUccFile = 0;
                }
                else
                {
                    ...
                    dataContext.FindFieldDataContext("DESCRIPTION").SetValue("UCC FILE");
                    dataContext.FindFieldDataContext("DESCRIPTION2").SetValue("WORKING FILE I SEC");
                    dataContext.FindFieldDataContext("DOCUMENT_TYPE").SetValue(ModCommon.ParseParam(dataContext.FindFieldDataContext("UCC_FILE").Text, 1, "-").Replace("\r", "").Trim());
                }
                ...
                intSelectedItems = intUccFile + intBorrowerInfo + intLegalDocuments + intNotCategorized;
        ...
        private void ParseBarCode(string BARCODE, IUimDataContext dataContext)
        {
            ...
            dataContext.FindFieldDataContext("UCC_FILE").SetValue(ModCommon.ParseParam(BARCODE, 14, "|").Replace("\r", "").Trim());
