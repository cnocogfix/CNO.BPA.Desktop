...
namespace Custom.InputAccel.UimScript
{
    ...
    public class ScriptMORTGAGEWORKINGFILE2 : UimScriptDocument
    {
        private const string _validationRule = "One";

        public ScriptMORTGAGEWORKINGFILE2()
            : base()
        {
        ...
        private void AutoValidation(IUimDataContext dataContext)
        {
            try
            {
                int intServicingFile;
                int intInspectionFile;
                int intOperatingStmt;
                int intAssumption;
                int intDraw;
                ...
                //validate if only one field among SERVICING_FILE, INSPECTION_FILE, OPERATING_STMT, ASSUMPTION, DRAW, NOT_CATEGORIZED is selected
                //The interger variable would be assigned with 0 if the value fetched is null otherWISE it will be 1
                //SERVICING_FILE
                if (dataContext.FindFieldDataContext("SERVICING_FILE").Text.Trim().Length == 0)
                {
                    intServicingFile = 0;
                }
                else
                {
                    ...
                    dataContext.FindFieldDataContext("DESCRIPTION").SetValue("SERVICING FILE");
                    dataContext.FindFieldDataContext("DESCRIPTION2").SetValue("WORKING FILE II SEC");
                    dataContext.FindFieldDataContext("DOCUMENT_TYPE").SetValue(ModCommon.ParseParam(dataContext.FindFieldDataContext("SERVICING_FILE").Text, 1, "-").Replace("\r", "").Trim()); 
                }

                //INSPECTION_FILE
                if (dataContext.FindFieldDataContext("INSPECTION_FILE").Text.Trim().Length == 0)
                {
                    intInspectionFile = 0;
                }
                else
                {
                    ...
                    dataContext.FindFieldDataContext("DESCRIPTION").SetValue("INSPECTION FILE");
                    dataContext.FindFieldDataContext("DESCRIPTION2").SetValue("WORKING FILE II SEC");
                    dataContext.FindFieldDataContext("DOCUMENT_TYPE").SetValue(ModCommon.ParseParam(dataContext.FindFieldDataContext("INSPECTION_FILE").Text, 1, "-").Replace("\r", "").Trim()); 
                }

                //OPERATING STMT
                if (dataContext.FindFieldDataContext("OPERATING_STMT").Text.Trim().Length == 0)
                {
                    intOperatingStmt = 0;
                }
                else
                {
                    ...
                    dataContext.FindFieldDataContext("DESCRIPTION").SetValue("OPERATING STATEMENT");
                    dataContext.FindFieldDataContext("DESCRIPTION2").SetValue("WORKING FILE II SEC");
                    dataContext.FindFieldDataContext("DOCUMENT_TYPE").SetValue(ModCommon.ParseParam(dataContext.FindFieldDataContext("OPERATING_STMT").Text, 1, "-").Replace("\r", "").Trim());
                }

                //ASSUMPTION
                if (dataContext.FindFieldDataContext("ASSUMPTION").Text.Trim().Length == 0)
                {
                    intAssumption = 0;
                }
                else
                {
                    ...
                    dataContext.FindFieldDataContext("DESCRIPTION").SetValue("ASSUMPTION");
                    dataContext.FindFieldDataContext("DESCRIPTION2").SetValue("WORKING FILE II SEC");
                    dataContext.FindFieldDataContext("DOCUMENT_TYPE").SetValue(ModCommon.ParseParam(dataContext.FindFieldDataContext("ASSUMPTION").Text, 1, "-").Replace("\r", "").Trim());
                }

                //ASSUMPTION
                if (dataContext.FindFieldDataContext("DRAW").Text.Trim().Length == 0)
                {
                    intDraw = 0;
                }
                else
                {
                    ...
                    dataContext.FindFieldDataContext("DESCRIPTION").SetValue("DRAW");
                    dataContext.FindFieldDataContext("DESCRIPTION2").SetValue("WORKING FILE II SEC");
                    dataContext.FindFieldDataContext("DOCUMENT_TYPE").SetValue(ModCommon.ParseParam(dataContext.FindFieldDataContext("DRAW").Text, 1, "-").Replace("\r", "").Trim());
                }
                ...
                intSelectedItems = intServicingFile + intInspectionFile + intOperatingStmt + intAssumption + intDraw + intNotCategorized;
        ...
        private void ParseBarCode(string BARCODE, IUimDataContext dataContext)
        {
            ...
            DateTime dateObject = Convert.ToDateTime(ModCommon.ParseParam(BARCODE, 17, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("DOCUMENT_DATE").SetValue(dateObject.ToString("yyyy/MM/dd"));//TEST
            dataContext.FindFieldDataContext("ACCOUNT_NUMBER").SetValue(ModCommon.ParseParam(BARCODE, 18, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("BORROWER_NAME").SetValue(ModCommon.ParseParam(BARCODE, 19, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("PROPERTY_NAME").SetValue(ModCommon.ParseParam(BARCODE, 20, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("PROPERTY_ADDRESS").SetValue(ModCommon.ParseParam(BARCODE, 22, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("PROPERTY_CITY").SetValue(ModCommon.ParseParam(BARCODE, 25, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("PROPERTY_STATE").SetValue(ModCommon.ParseParam(BARCODE, 26, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("FULL_NAME").SetValue(ModCommon.ParseParam(BARCODE,27, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("SERVICING_FILE").SetValue(ModCommon.ParseParam(BARCODE, 21, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("INSPECTION_FILE").SetValue(ModCommon.ParseParam(BARCODE, 16, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("OPERATING_STMT").SetValue(ModCommon.ParseParam(BARCODE, 28, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("ASSUMPTION").SetValue(ModCommon.ParseParam(BARCODE, 29, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("DRAW").SetValue(ModCommon.ParseParam(BARCODE, 30, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("NOT_CATEGORIZED").SetValue(ModCommon.ParseParam(BARCODE, 23, "|").Replace("\r", "").Trim());
        }
