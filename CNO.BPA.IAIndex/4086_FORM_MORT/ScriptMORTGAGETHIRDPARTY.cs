...
namespace Custom.InputAccel.UimScript
{
    ...
    public class ScriptMORTGAGETHIRDPARTY : UimScriptDocument
    {
        private const string _validationRule = "One";

        public ScriptMORTGAGETHIRDPARTY()
            : base()
        {
        ...
        private void AutoValidation(IUimDataContext dataContext)
        {
            try
            {
                ...
                //validate if only one field among THIRD_PARTY_REPORT, NOT_CATEGORIZED is selected
                //The interger variable would be assigned with 0 if the value fetched is null otherWISE it will be 1
                if (dataContext.FindFieldDataContext("THIRD_PARTY_REPORT").Text.Trim().Length == 0)
                {
                    intThirdPartyReport = 0;
                }
                else
                {
                    intThirdPartyReport = 1;

                    dataContext.FindFieldDataContext("DESCRIPTION").SetValue("THIRD PARTY REPORTS");
                    dataContext.FindFieldDataContext("DESCRIPTION2").SetValue("THIRD PARTY REPORTS SEC");
                    dataContext.FindFieldDataContext("DOCUMENT_TYPE").SetValue(ModCommon.ParseParam(dataContext.FindFieldDataContext("THIRD_PARTY_REPORT").Text, 1, "-").Replace("\r", "").Trim()); 
                }
                ...
                intSelectedItems = intThirdPartyReport + intNotCategorized;
        ...
        private void ParseBarCode(string BARCODE, IUimDataContext dataContext)
        {
            ...
            dataContext.FindFieldDataContext("THIRD_PARTY_REPORT").SetValue(ModCommon.ParseParam(BARCODE, 12, "|").Replace("\r", "").Trim());
