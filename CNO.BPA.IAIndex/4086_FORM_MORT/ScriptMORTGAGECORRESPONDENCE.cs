...
namespace Custom.InputAccel.UimScript
{
    ...
    public class ScriptMORTGAGECORRESPONDENCE : UimScriptDocument
    {
        private const string _validationRule = "One";

        public ScriptMORTGAGECORRESPONDENCE()
            : base()
        {
        ...
        private void AutoValidation(IUimDataContext dataContext)
        {
            try
            {
                int intOriginationCorr;
                int intServicingCorr;
                ...
                //validate if only one field among ORIGINATION_CORR, SERVICING_CORP, NOT_CATEGORIZED is selected
                //The interger variable would be assigned with 0 if the value fetched is null otherWISE it will be 1
                if (dataContext.FindFieldDataContext("ORIGINATION_CORR").Text.Trim().Length == 0)
                {
                    intOriginationCorr = 0;
                }
                else
                {
                    intOriginationCorr = 1;

                    dataContext.FindFieldDataContext("DESCRIPTION").SetValue("ORIGINATION_CORR");
                    dataContext.FindFieldDataContext("DESCRIPTION2").SetValue("CORRESPONDENCE SEC");
                    dataContext.FindFieldDataContext("DOCUMENT_TYPE").SetValue(ModCommon.ParseParam(dataContext.FindFieldDataContext("OFFERING").Text, 1, "-").Replace("\r", "").Trim()); 
                }
                //SERVICING CORP
                if (dataContext.FindFieldDataContext("SERVICING_CORR").Text.Trim().Length == 0)
                {
                    intServicingCorr = 0;
                }
                else
                {
                    ...
                    dataContext.FindFieldDataContext("DESCRIPTION").SetValue("SERVICING CORR");
                    dataContext.FindFieldDataContext("DESCRIPTION2").SetValue("CORRESPONDENCE SEC");
                    dataContext.FindFieldDataContext("DOCUMENT_TYPE").SetValue(ModCommon.ParseParam(dataContext.FindFieldDataContext("SERVICING_CORR").Text, 1, "-").Replace("\r", "").Trim()); 
                }
                ...
                intSelectedItems = intOriginationCorr + intServicingCorr + intNotCategorized;
        ...
        private void ParseBarCode(string BARCODE, IUimDataContext dataContext)
        {
            ...
            dataContext.FindFieldDataContext("ACCOUNT_NUMBER").SetValue(ModCommon.ParseParam(BARCODE, 17, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("BORROWER_NAME").SetValue(ModCommon.ParseParam(BARCODE, 23, "|").Replace("\r", "").Trim());
            ...
            dataContext.FindFieldDataContext("PROPERTY_CITY").SetValue(ModCommon.ParseParam(BARCODE, 21, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("PROPERTY_STATE").SetValue(ModCommon.ParseParam(BARCODE, 22, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("FULL_NAME").SetValue(ModCommon.ParseParam(BARCODE,24, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("ORIGINATION_CORR").SetValue(ModCommon.ParseParam(BARCODE, 16, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("SERVICING_CORR").SetValue(ModCommon.ParseParam(BARCODE, 19, "|").Replace("\r", "").Trim());
            dataContext.FindFieldDataContext("NOT_CATEGORIZED").SetValue(ModCommon.ParseParam(BARCODE, 14, "|").Replace("\r", "").Trim());
        }
